...
def main(arg=None):
    ...
    def compute_metrics(pred):
        ...
        pred_ids = [i[i != -100] for i in pred_ids] #UNK
        pred_ids = [i[i != -101] for i in pred_ids] #CLS
        pred_ids = [i[i != -102] for i in pred_ids] #SEP
        pred_ids = [i[i != 0] for i in pred_ids] #PAD

        label_ids = pred.label_ids
        label_ids = [i[i != -100] for i in label_ids]
        label_ids = [i[i != -101] for i in label_ids]
        label_ids = [i[i != -102] for i in label_ids]
        label_ids = [i[i != 0] for i in label_ids]


        pred_str = processor.tokenizer.batch_decode(pred_ids, skip_special_tokens=False, group_tokens=False, spaces_between_special_tokens=True)
        label_str = processor.tokenizer.batch_decode(label_ids, skip_special_tokens=False, group_tokens=False, spaces_between_special_tokens=True)

        # # for cal wer need to adding spaces_between_special_tokens=True, if cer spaces_between_special_tokens=False
        # pred_str_space = processor.tokenizer.batch_decode(pred_ids, skip_special_tokens=True, group_tokens=False, spaces_between_special_tokens=True)
        ...
        # label_str_space = processor.tokenizer.batch_decode(label_ids, skip_special_tokens=True, group_tokens=False, spaces_between_special_tokens=True)

        cer = cer_cal(label_str, pred_str)
        wer = wer_cal(label_str, pred_str)
        # wer = wer_cal(label_str_space, pred_str_space)
    ...
    class Wav2Vec2ForCTC(Wav2Vec2PreTrainedModel):
        def __init__(self, config):
            ...
            # print(config.apply_spec_augment)
            # input()
    ...
    if 'custom_set' in input_arg:
        ...
        cache_file_test = f"{input_arg['custom_set_test']}_hf_test.data"
        print(cache_file_train)
        ...
        if os.path.isdir(cache_file_train) and os.path.isdir(cache_file_test):
            ...
            data_test = load_dataset('csv', data_files=input_arg['custom_set_test'])['train']
            data_train = data_train.load_from_disk(cache_file_train)
            data_test = data_test.load_from_disk(cache_file_test)
        else:
            # dataset = load_dataset('csv', data_files=input_arg['custom_set'], delimiter="," , cache_dir='./.cache')
            # dataset = dataset['train']
            # dataset = dataset.train_test_split(test_size=0.1)
            # data_train = dataset['train']
            # data_test = dataset['test']

            data_train = load_dataset('csv', data_files=input_arg['custom_set_train'], delimiter="," , cache_dir='./.cache')
            data_test = load_dataset('csv', data_files=input_arg['custom_set_test'], delimiter="," , cache_dir='./.cache')
